...
def set_fixed_exp_params(N, M, K,N_list,M_list,K_list,tot,R):
  ...
  path = Path.cwd().joinpath('parameter_files', 'v4', 'de_dg_gw_upper.xlsx')
  ...
  path = Path.cwd().joinpath('parameter_files', 'v4', 'de_dg_gwq_lower.xlsx')
  df = pd.read_excel(path) #lower bounds for de_dg for sw
  gwq_lower = df.fillna(0).values[:,1:]
  gwq_lower = np.array(gwq_lower, dtype=[('O', float)]).astype(float)
  path = Path.cwd().joinpath('parameter_files', 'v4', 'de_dg_gwq_upper.xlsx')
  df = pd.read_excel(path)
  ...
  path = Path.cwd().joinpath('parameter_files', 'v4', 'dt_dh.xlsx')
